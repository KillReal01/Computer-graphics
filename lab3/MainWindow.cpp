    ...
    , ui(new Ui::MainWindow)
{
    ...
    center = Point(delta + length, length, delta + length);

    line_x = new Figure(std::vector<Edge<Point>>{Edge<Point>(center, {delta + length * 2, length, delta + length})},
                        Point(delta + length, length, delta + length), "X", QPen(Qt::black, 2, Qt::SolidLine));
    line_y = new Figure(std::vector<Edge<Point>>{Edge<Point>(center, {delta + length, length * 2, delta + length})},
                        Point(delta + length, length, delta + length), "Y", QPen(Qt::black, 2, Qt::SolidLine));
    line_z = new Figure(std::vector<Edge<Point>>{Edge<Point>(center, {delta + length, length, delta + length * 2})},
                        Point(delta + length, length, delta + length), "Z", QPen(Qt::black, 2, Qt::SolidLine));


    for (auto& fig : {figure_base, figure_bezier})
    {
        fig->rotationOY(10);
        fig->rotationOX(-60);
        fig->rotationOZ(180);
        ui->canvas->DrawItem(fig);
    }
...
void MainWindow::createBaseFigure()
{
    ...
    //Point c(500, 300, 100);
    Point c(scalefig * row / 2, scalefig * column / 2, scalefig / 2);
    //c = {0, 0, 0, 0};
    figure_base = new Figure(edges_base, c, "Base surface", QPen(Qt::blue, 1, Qt::SolidLine));
}
...
void MainWindow::createBezierFigure(int accuracy)
{
    ...
    //Point c(500, 300, 100);
    Point c(scalefig * row / 2, scalefig * column / 2, scalefig / 2);
    figure_bezier = new Figure(edges_bezier, c, "Bezier surface", QPen(Qt::red, 1, Qt::SolidLine));
}
